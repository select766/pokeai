...
def load_parties_agents(env: PokeEnv, party_file_path: str):
    ...
    return parties, action_samplers
...
class PartyTrainEvaluator:
    ...
    def train_and_evaluate(self, friend_party: Party, baseline_rate: float, outdir: str) -> float:
        ...
        rate = self.rating_single_party(env, self.enemy_pool, self.enemy_agents, self.enemy_pool_rate, self.match_count,
                                        agent.act,
                                        baseline_rate, baseline_rate - 400.0)
        return rate

    def match_policy(self, env: PokeEnv, enemy_party: Party, enemy_agent: Callable[[np.ndarray], int],
                     action_sampler: Callable[[np.ndarray], int]) -> int:
        obs = env.reset(enemy_party, enemy_agent)
        done = False
        reward = 0
        while not done:
    ...
    def rating_single_party(self, env: PokeEnv, parties: List[Party], agents: List[Callable[[np.ndarray], int]],
                            party_rates: np.ndarray, match_count: int,
        ...
        for i in range(match_count):
            ...
            winner = self.match_policy(env, parties[nearest_party_idx], agents[nearest_party_idx], action_sampler)
            # レートを変動させる
            if winner >= 0:
...
def main():
    ...
    parser.add_argument("baseline_party_rl_file", help="レーティング測定相手パーティ群(強化学習)")
