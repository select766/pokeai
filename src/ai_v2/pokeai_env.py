...
class PokeaiEnv():
    ...
    def get_possible_action_vec(self, player):
        ...
        if self.field.phase is pokeai_simu.FieldPhase.Begin:
            ...
            if f_poke.move_handler is not None:
                ...
                #何番目の技かに変換
                move_idx = f_poke.static_param.move_ids.index(move_id)
                assert move_idx >= 0
                vec[move_idx] = 1
            else:
                #すべての技+生きている控え
    ...
    def get_fighting_poke_vec(self, poke):
        ...
        vec[0] = int(not poke.is_faint)
        vec[1] = poke.hp / 1024.0
        vec[2] = poke.static_param.max_hp / 1024.0
        #状態異常６種類
        vec[3 + poke.nv_condition.value - pokeai_simu.PokeNVCondition.Empty.value] = 1
        vec[9] = int(poke.confusion_turn > 0)
        vec[10] = int(poke.bad_poison)
        vec[11] = min(poke.bad_poison_turn / 16.0, 1.0)
        vec[12] = int(poke.reflect)
        vec[13] = int(poke.digging)
        vec[14] = poke.rank_a / 12.0 + 0.5
        vec[15] = poke.rank_b / 12.0 + 0.5
        vec[16] = poke.rank_c / 12.0 + 0.5
        vec[17] = poke.rank_s / 12.0 + 0.5
        vec[18] = poke.rank_accuracy / 12.0 + 0.5
        vec[19] = poke.rank_evasion / 12.0 + 0.5
        return vec
    ...
    def _action_to_field_action(self, action):
        if action < N_MOVES:
            return pokeai_simu.FieldActionBegin.move(action)
    ...
    def _sample_enemy_action(self):
        ...
        enemy_fa = self._action_to_field_action(enemy_corrected_action)
        return enemy_fa
    ...
    def step(self, action):
        ...
        if self.field.phase is pokeai_simu.FieldPhase.Begin:
            ...
            friend_fa = self._action_to_field_action(corrected_action)
